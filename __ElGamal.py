...
class ElGamalKey(object):
    ...
    def encrypt(self, plaintext, K):
        ciphertext=self._encrypt(int.from_bytes(plaintext, "big"), K)
        return ciphertext

    def decrypt(self, ciphertext):
        plaintext=self._decrypt(ciphertext)
        return plaintext.to_bytes(length=(plaintext.bit_length() + 1)//8 ,byteorder='big')
...
def read_elgamal_pem_file(pem_file_path):
    ...
    if is_public == b'-----BEGIN ELGAMAL PUBLIC KEY-----\n':
        ...
        size = 1024 if len(lines[1:-1]) == 8 else 2048
        print(len(lines[1:-1]))
        ...
        return construct((p,g,y)), size

    else:
        size = 1024 if len(lines[1:-1]) == 10 else 2048
        print(len(lines[1:-1]))
